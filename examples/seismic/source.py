...
class PointSource(SparseTimeFunction):
    ...
    def resample(self, dt, window=None):
        ...
        # The new number of sample points and roundoff adjusted dt. This
        # uses ceil as we need to ensure that after we correct dt for
        # the remainder we get a dt less than or equal to the requested one
        # and thereby avoid exceeding the critical dt.
        nt = int(np.ceil((stop - start + dt)/dt))
        ...
        data = np.zeros((nt, npoint))
        for i in range(npoint):
            data[:, i] = signal.resample(self.data[:, i], nt, window=window)

        # Return new object
        new_time_range = TimeAxis(start=start, stop=stop, num=nt)
